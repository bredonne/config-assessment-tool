...
class DashboardsBRUM(JobStepBase):
    def __init__(self):
        super().__init__("brum")
        ...
        for host, hostInfo in controllerData.items():
            ...
            for application in hostInfo[self.componentType].values():
                ...
                for dashboard in application["biqDashboards"]:
                    ...
                    if num_months <= 6:
                        numDashboardsModifiedLast6Months += 1
                if len(application["brumDashboards"]) + len(application["biqDashboards"]) == 0:
                    ...
                    analysisDataEvaluatedMetrics["percentageOfDashboardsModifiedLast6Months"] = (
                        numDashboardsModifiedLast6Months / (len(application["brumDashboards"]) + len(application["biqDashboards"])) * 100
                    )
